...
public class MainCauldronListener implements Listener
{
            ...
            if (CauldronWitcheryAddon.MAGIC_SUMMON_ENABLE_FLAG.isSetForWorld(block.getWorld()))
            {
                ...
                if (island.isPresent())
                {
                    ...
                    if (island.get().isAllowed(user, CauldronWitcheryAddon.MAGIC_SUMMON_ISLAND_PROTECTION))
                    {
                        ...
                        if (!block.getBlockData().getAsString().equals("minecraft:" + CAULDRON.name().toLowerCase(Locale.ENGLISH) + "[level=3]"))
                        {
    ...
    private static final Material CAULDRON = Enums.getIfPresent(Material.class, "WATER_CAULDRON").or(Material.CAULDRON);
